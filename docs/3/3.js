...
function toState2(timestamp) {
    ...
    // interpolate between current user point position and next position
    for (let i = 0; i < userPoints.length; i++) {
        for (let j = 0; j < userPoints[i].length; j++) {
            userPoints[i][j].x = (1 - easedU) * state1UserPoints[i][j].x + easedU * state2UserPoints[i][j].x;
            userPoints[i][j].y = (1 - easedU) * state1UserPoints[i][j].y + easedU * state2UserPoints[i][j].y;
        }
    }
}
...
function toState1(timestamp) {
    ...
    for (let i = 0; i < userPoints.length; i++) {
        for (let j = 0; j < userPoints[i].length; j++) {
            userPoints[i][j].x = (1 - easedU) * state2UserPoints[i][j].x + easedU * state1UserPoints[i][j].x;
            userPoints[i][j].y = (1 - easedU) * state2UserPoints[i][j].y + easedU * state1UserPoints[i][j].y;
        }
    }
}
...
// transition button --- determines if animation occurs
const transitionButton = document.getElementById("transitionButton");
// start unchecked
transitionButton.checked = false;

transitionButton.addEventListener("click", () => {
    // if button is checked, start animation
    if(transitionButton.checked) {
        // start animation
        window.requestAnimationFrame(animate);
    }
});

// state 1 button --- saves the current grid state
const state1Button = document.getElementById("state1Button");
let state1 = [];
let state1UserPoints = [];

state1Button.addEventListener("click", () => {
    // reset
    state1 = [];
    state1UserPoints = [];

    // copy the current grid state for grid control points
    for (let i = 0; i < GRID_ROWS; i++) {
        state1[i] = [];

        for (let j = 0; j < GRID_COLS; j++) {
            state1[i][j] = { ...controlPoints[i][j] };
        }
    }
    // copy the current grid state for user points
    for (let i = 0; i < userPoints.length; i++) {
        state1UserPoints[i] = [];

        for (let j = 0; j < userPoints[i].length; j++) {
            state1UserPoints[i][j] = { ...userPoints[i][j] };
        }
    }
});

// state 2 button --- saves the current grid state
const state2Button = document.getElementById("state2Button");
let state2 = [];
let state2UserPoints = [];

state2Button.addEventListener("click", () => {
    // reset
    state2 = [];
    state2UserPoints = [];
    ...
    for (let i = 0; i < userPoints.length; i++) {
        ...
        for (let j = 0; j < userPoints[i].length; j++) {
            state2UserPoints[i][j] = { ...userPoints[i][j] };
        }
    }
});
...
function setControlPoints() {
    ...
    for (let i = 0; i < GRID_ROWS; i++) {
        ...
        for (let j = 0; j < GRID_COLS; j++) {
            ...
            // set up state1 and state2 to be the same as control points as the default
            state1[i][j] = { ...controlPoints[i][j] };
            state2[i][j] = { ...controlPoints[i][j] };
        }
    }
    ...
    for (let i = 0; i < GRID_ROWS; i++) {
        ...
        for (let j = 0; j < GRID_COLS; j++) {
            if ( j === GRID_COLS - 1 || i === GRID_ROWS - 1) {
                continue;
            }
            ...
            // set up state1 and state2 to be the same as user points as the default
            state1UserPoints[i][j] = { ...userPoints[i][j] };
            state2UserPoints[i][j] = { ...userPoints[i][j] };
        }
    }
...
function animate(timestamp) {
    // if transition button is not checked, stop the animation
    if (!transitionButton.checked) {
        return;
    }
...
canvas.addEventListener("mousedown", (e) => {
  // if animation is occuring, do not do anything
  if (transitionButton.checked) {
    return;
  }
...
canvas.addEventListener("mousemove", (e) => {
  ...
  if (dragging) {
    ...
    if (dragging.isControl) {
        ...
        const prevX = userPoints[dragging.i][dragging.j].x;
        const prevY = userPoints[dragging.i][dragging.j].y;
        // get new user point location
        userPoints[dragging.i][dragging.j].x = mx;
        userPoints[dragging.i][dragging.j].y = my;

        // get change
        const dx = mx - prevX;
        const dy = my - prevY;
        // update the control points surrounding it based on the change in user point location
        controlPoints[dragging.i][dragging.j].x += dx;
        controlPoints[dragging.i][dragging.j].y += dy;
        controlPoints[dragging.i + 1][dragging.j].x += dx;
        controlPoints[dragging.i + 1][dragging.j].y += dy;
        controlPoints[dragging.i][dragging.j + 1].x += dx;
        controlPoints[dragging.i][dragging.j + 1].y += dy;
        controlPoints[dragging.i + 1][dragging.j + 1].x += dx;
        controlPoints[dragging.i + 1][dragging.j + 1].y += dy;
    }
